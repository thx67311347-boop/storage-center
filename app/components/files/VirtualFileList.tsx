...
export default function VirtualFileList({ files, onFileClick, onFileDelete, onFileDownload, onFileRename, onFileRestore, onFileShare, onMultiFileShare, onMultiFileDelete, selectedFiles, onSelectFile, isTrash }: FileListProps) {
  ...
  const FileItemComponent = ({ index, style }: { index: number; style: React.CSSProperties }) => {
        ...
        {sortedFiles.length > 0 ? (
          <div className="divide-y divide-gray-200 dark:divide-gray-800 max-h-[600px] overflow-y-auto">
            {sortedFiles.map((file, index) => (
              <FileItemComponent key={file.id} index={index} style={{}} />
            ))}
          </div>
        ) : (
