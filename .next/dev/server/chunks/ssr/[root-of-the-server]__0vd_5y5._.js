    ...
    var handleFilesUploaded = /*#__PURE__*/ function() {
        var _ref = _asyncToGenerator(function*(uploadedFiles, currentFolder, files, updateFiles, updateUsedStorage) {
            try {
                ...
                // 过滤掉超过大小限制的文件
                var validFiles = uploadedFiles.filter((file)=>{
                    if (file.size > MAX_FILE_SIZE) {
                        alert("\u6587\u4EF6 \"".concat(file.name, "\" \u8D85\u8FC7\u4E8610MB\u7684\u5927\u5C0F\u9650\u5236\uFF0C\u65E0\u6CD5\u4E0A\u4F20\u3002\n\n\u63D0\u793A\uFF1AlocalStorage\u7684\u5BB9\u91CF\u6709\u9650\uFF0C\u5EFA\u8BAE\u4E0A\u4F20\u8F83\u5C0F\u7684\u6587\u4EF6\u3002"));
                        return false;
                    }
                    return true;
                });
...
var useMegaStorage = ()=>{
        ...
        var connectToMega = /*#__PURE__*/ function() {
            var _ref = _asyncToGenerator(function*() {
                try {
                    ...
                    setError('Failed to connect to Mega. Please check your credentials.');
    ...
    var uploadFile = /*#__PURE__*/ function() {
        var _ref2 = _asyncToGenerator(function*(file) {
            if (!megaStorage || !isConnected) {
                setError('Not connected to Mega');
                return null;
            }
            try {
                var fileName = file.name;
                var fileSize = file.size;
                console.log("Uploading file: ".concat(fileName, " (").concat(fileSize, " bytes)"));
                // 创建文件上传流
                ...
                upload.on('progress', (progress)=>{
                    console.log("Upload progress: ".concat(Math.round(progress * 100), "%"));
                });
                // 写入文件数据
                var arrayBuffer = yield file.arrayBuffer();
                // 使用正确的方法上传文件
                upload.write(Buffer.from(arrayBuffer));
                upload.end();
                // 等待上传完成
                ...
                setError('Failed to upload file to Mega');
                return null;
            }
        });
    ...
    var downloadFile = /*#__PURE__*/ function() {
        var _ref3 = _asyncToGenerator(function*(fileLink) {
            ...
            try {
                console.log("Downloading file from: ".concat(fileLink));
                // 使用正确的方法获取文件
                ...
                console.error('Failed to download file from Mega:', err);
                setError('Failed to download file from Mega');
                return null;
            }
        });
    ...
    var deleteFile = /*#__PURE__*/ function() {
        var _ref4 = _asyncToGenerator(function*(fileLink) {
            ...
            try {
                console.log("Deleting file: ".concat(fileLink));
                // 使用正确的方法获取文件
                var file = megaStorage.file(fileLink);
                yield file.delete();
                ...
                setError('Failed to delete file from Mega');
                return false;
            }
        });
...
function FileManager() {
    ...
    // Mega存储钩子
    var { isConnected, uploadFile: uploadFileToMega, error: megaError } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$hooks$2f$useMegaStorage$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useMegaStorage"])();
    // 上传状态
    ...
    var handleFilesUploadedWrapper = /*#__PURE__*/ function() {
        var _ref = _asyncToGenerator(function*(uploadedFiles) {
            // 对大文件使用Mega存储
            var largeFiles = uploadedFiles.filter((file)=>file.size > 10 * 1024 * 1024); // 10MB以上
            var smallFiles = uploadedFiles.filter((file)=>file.size <= 10 * 1024 * 1024);
            // 处理小文件
            if (smallFiles.length > 0) {
                yield handleFilesUploaded(smallFiles, currentFolder, files, (updatedFiles)=>dispatch({
                        type: 'SET_FILES',
            ...
            // 处理大文件（使用Mega）
            if (largeFiles.length > 0 && isConnected) {
                setIsUploadingToMega(true);
                try {
                    for (var file of largeFiles){
                        var megaLink = yield uploadFileToMega(file);
                        if (megaLink) {
                            // 创建文件对象并添加到列表
                            var newFile = {
                                id: "mega_".concat(Date.now(), "_").concat(Math.random().toString(36).substr(2, 9)),
                                name: file.name,
                                size: file.size,
                                type: file.type,
                                url: megaLink,
                    ...
                    console.error('Error uploading files to Mega:', error);
                    alert('部分大文件上传失败，请重试。');
                } finally{
                    setIsUploadingToMega(false);
                }
            } else if (largeFiles.length > 0 && !isConnected) {
                alert('Mega存储未连接，无法上传大文件。');
            }
        });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0vd_5y5._.js.map